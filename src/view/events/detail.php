  <main>
      <header>
        <a href="index.php?page=lineup">keer terug naar overzicht</a>
        <h1>The Sky Is The Limit</h1>
        <p>door Flying Dutchmen</p>
      </header>
      <div>
        <section>
          <h2 class="hidden">headerinfo</h2>
          <p>#voorstelling</p>
          <dl>
            <dt><img src="../assets/img/icon/calendar.svg" alt="" /></dt>
            <dd>vrijdag</dd>

            <dt><img src="../../assets/img/icon/clock.svg" alt="" /></dt>
            <dd>
              18:00-18:40<br/>
                20:15-20:55
            <dd>

            <dt><img src="../assets/img/icon/location.svg" alt="" /></dt></dt>
            <dd>Speelplaats KA/BS De Bevers</dd>

            <dt><img src="../assets/img/icon/nationality.svg" alt="" /></dt></dt>
            <dd>Nederland & Canada</dd>

            <dt><img src="../assets/img/icon/globe.svg"" alt="" /></dt></dt>
            <dd><a href="http://www.the-flying-dutchmen.com/">website</a></dd>
          </dl>
          <p>
              De Amsterdamse jongleur Michiel
              Hesseling, "...<span class="highlight">een Mozart in zijn beroep</span>..."
              genoemd, en Canada's "...meest
              spectaculaire éénwielfietser..." Jean-Michel Paré vormden in 1989 een inmiddels wereldberoemd duo.
          </p>
          <p>
              Deze twee “old school” <span class="highlight">komische genieën</span> worden als helden beschouwd in de
              internationale wereld van straatartiesten.
          </p>
          <a href="">lees meer...</a>
        </section>
        <section>
          <h2>Onze Video's</h2>
          <div>
            <ul>
              <li><a href="https://youtu.be/8jKNGl-TwXc"><img src="" alt=""></a></li>
              <li><a href="https://youtu.be/gjPItA8fqsM"><img src="" alt=""></a></li>
            </ul>
          </div>
        </section>

        <section>
          <h2>Onze Foto's</h2>
          <div>
            <ul>
              <li><img src="" alt=""></li>
              <li><img src="" alt=""></li>
              <li><img src="" alt=""></li>
              <li><img src="" alt=""></li>
            </ul>
          </div>
        </section>

        <section>
          <h2>Ook op deze dag</h2>
          <ul>
            <li><a href=""></a>Cie Xav To Yilo</li>
            <li><a href=""></a>Cie Sitting Duck</li>
            <li><a href=""></a>Krijmfresj</li>
          </ul>
        </section>
      </div>
    </main>

